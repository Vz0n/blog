...
Dada esta pequeña introducción y explicación de los términos que estaré usando podemos comenzar.

<h2>Enumeración</h2>

El servidor corre normalmente en el puerto 25565, algunos lo suelen colocar en rangos de puertos ya sea para evitar colisiones (Hostings), o para "esconderlo" de atacantes.

Al enviar un ping de cliente al servidor, este te reporta la cantidad de jugadores y el software del servidor; este último es editado por varios servidores para evitar enumeración.
...
Ya teniendo en claro como puede estar estructurado el servidor, vamos a explorar fallos que se pueden explotar.
...
¿Podemos llevar este fallo a otro nivel además de simplemente spoofear la UUID de una cuenta con muchos permisos y buscar formas de elevar nuestro control?, depende mucho de los plugins que tengan instalados y como los tienen configurados.

**SOLUCIÓN:** Tienes dos formas de solucionar este problema

- Firewall: Si tienes un dedicado o VPS puedes establecer reglas de Firewall para impedir la entrada sin autorización a tus servidores conectados en la network desde el Proxy del atacante. Para ello obviamente debes tener un software de firewall instalado. Es muy recomendable que configures el firewall para que solo permita conexiones que coinciden con una regla que permite el acceso a un puerto especifico, hay varias guías por internet para poder hacer esto y también existe la [guía oficial de SpigotMC](https://www.spigotmc.org/wiki/firewall-guide/).
...
- Plugins: Usando plugins puedes llegar a replicar algo parecido a un firewall haciendo que las instancias solo acepten conexiones de un BungeeCord que esté en cierta dirección o que presente un token o nonce en el handshake de la conexión. Los plugins más conocidos hasta ahora de este estilo es [BungeeGuard](https://github.com/lucko/BungeeGuard) y [SafeNET](https://github.com/dejvokep/safe-net), el problema con esto es que la instancia sigue aceptando la conexión y solamente deniega la entrada al servidor de Minecraft en sí, por lo que podrían meter bots o... utilizar algún fallo o desconfiguración que tenga el plugin para ganar acceso.
...
El programador, pensando que uno no podría poner chars raros en la URI colocó la siguiente pieza de código por flojera:
...
De aquí puedo volcarme toda la base de datos y, si el usuario que se usa para efecutar las peticiones SQL tiene acceso a otras bases de datos como puede ser la de AuthMe o tus otros plugins me las puedo volcar también, y si usara PostgreSQL para manejar todo podría llegar hasta ejecutar comandos en el servidor. Dependiendo de las bases de datos y el contenido en estas podría llegar a obtener permisos administrativos en el servidor de Minecraft, y si la SQLi está en una query que altera valores más probabilidades tendrás de darte dichos permisos.

**SOLUCIÓN:** Simplemente usa el sentido común y sigue los consejos y recomendaciones de seguridad a la hora de hacer sitios web u otro tipo de servicios externos que se van a comunicar con tu servidor de Minecraft.

<h4>¿Quién y cómo ejecuta el servidor?</h4>

Hay que fijarse bien en como alguien está ejecutando su servidor o toda su network de Minecraft por seguridad y escalabilidad.

El servidor de Minecraft se ejecuta como un proceso normal del sistema, y ese proceso lo ejecuta un usuario, usuario que tiene permisos, cualquier comando o cambio que se haga en el sistema de archivos estará a nombre de ese usuario; lo que quiere decir que si ejecuto el servidor como un administrador o root todo lo que haga el servidor se hará con permisos de dicho usuario. Es por esto que viven diciendo que estando en Linux, nunca ejecutes tus servidores como el usuario root.

Si quieres ver que tan peligroso es esto, vamos a ponerte un contexto de ejemplo de un plugin custom que un desarrollador que trabaja para un servidor llamado "Mailbox Network" creó para ejecutar comandos del sistema ya que por alguna razón le era molesto entrar por SSH y usar comandos de Linux para meterse en el screen o contenedor del servidor.
...
En instalaciones de servidores en Windows Server igual hay que tener cuidado, debes evitar usar el usuario `Administrador` para correr el servidor y sigue los consejos de seguridad de dicho sistema operativo, si es que por alguna razón usan dicho sistema para correr el proyecto de Minecraft.

**RECOMENDACIÓN:** Corre los servidores en contenedores bien protegidos o bajo un usuario que no tenga ningún tipo de permiso especial. También mantén actualizado tu sistema, programas y librerias.

<h4>Ingeniería social y OSINT</h4>

Pedirle contraseñas fuertes y cuentas premium a la administración no sirve de mucho si la misma se deja engañar fácilmente por atacantes.
...
Con buscadores como Shodan o Censys se puede indagar para encontrar servidores con servicios interesantes.

![Server search engines](/assets/posts/minecraft-security/censys.png)

Con esta forma de búsqueda obviamente alguien podría obtener tu dirección de correo personal o nombre real para poder intentar hacerse tus cuentas mediante Phishing.
...
<h4>Plugins, mods y el software del servidor</h4>

No cambiar casi nada de la configuración de los plugins o instalar plugins mal hechos/desactualizados puede no solo introducir problemas de estabilidad y escalabilidad, si no también de seguridad.
...
También existen varios errores por parte de los configuradores de servidores y desarrolladores de plugins que puedes usar para obtener permisos de operador o bien enumerar el sistema que utiliza el servidor para correr, y hay funciones propias de los plugins que puedes usar para lograr lo último dicho o algo peor; aquí mostraré unos cuantos casos y funciones de plugins que te pueden dar una idea.

<h4>Plugins con funciones de respaldos, carga de archivos... etc</h4>

Algunos de estos tipos de plugins te pueden permitir hacer cosas interesantes, pero otros solo pueden servir para reconocimiento.

Por ejemplo, al exportar los permisos de LuckPerms, este plugin te dice donde guarda el archivo o donde se debería encontrar lo que intenta cargar en forma de ruta absoluta. Con esto puedes llegar a saber quien o donde se ejecuta el servidor.

![LuckPerms](/assets/posts/minecraft-security/luckperms.png)

<h4>BungeeServerManager y derivados</h4>
...
<h4>ServerUtils, PlugManX... etc</h4>
...
<h4>Plugins con sistemas para subir dumps, logs... etc</h4>
...
<h4>Plugins que hacen peticiones</h4>

Estos plugins son aquellos que normalmente te permiten descargar configuraciones o recursos de la web que le especifiques, podrías usar esto para filtrar la dirección IPv4 verdadera del servidor si está detrás de un proxy reverso o un servicio como TCPShield/Infinity Guard o para enumerar puertos internos si es que te permite hacer peticiones a la misma máquina. Un ejemplo de estos plugins es el conocido CommandPanels.

<h4>PlaceholderAPI</h4>

Este plugin de placeholders te permite descargar expansiones de un sitio llamado "eCloud", ciertas de sus expansiones como "Server" y "Pinger" pueden ser usadas para enumeración o filtrar información. Pueden haber otras pero debes buscarlas.

<h4>Plugins desactualizados</h4>

Si un servidor tiene plugins desactualizados, puedes buscar entre sus versiones a ver si la que corre el servidor tiene una vulnerabilidad sin parchar, como el Path Traversal de HolographicDisplays en versiones anteriores a la 2.2.9, el comando `sqlexec` de Litebans que existe en versiones antiguas o el /promote y /demote de PermissionsEx.

<h4>Módulos de BungeeCord por defecto</h4>
...
Si un servidor tiene esto puesto junto al módulo cmd_server, podrías usar el comando /server para saltarte el sistema de autenticación del servidor si este tiene uno débil y usa el sistema de permisos por defecto de BungeeCord. También puedes probar a entrar con el nick "md_5" e intentar saltarte la autenticación o enviar a otra cuenta fuera del servidor de autenticación.

<h4>PlayerAnalytics, WebServer... etc</h4>

Los plugins de este estilo abren un servicio web que puede ser visto en puertos como el 8080, 8000 o el mismo 80 y 443, pueden tener comandos para agregar archivos a la web o usuarios. (como `/plan register` de PlayerAnalytics).

<h4>BungeeCommands</h4>

Si un servidor tiene un plugin de este estilo y solamente tienes el control de una modalidad, te puede ayudar para tomar control de todo el servidor, o como le dicen los "griefers"... "All network"

<h4>Spigot, Paper o BungeeCord muy desactualizados</h4>
